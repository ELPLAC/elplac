...
export class FairsController {
  ...
  // ✅ Nuevo endpoint para concluir feria
  @Roles(Role.ADMIN)
  @UseGuards(AuthGuard, RoleGuard)
  @Delete(':id')
  async deleteFair(@Param('id') fairId: string, @Req() req: any) {
    await this.fairsService.deleteFair(fairId, req.user.id);
    return { message: 'Feria concluida correctamente' };
  }
}
