...
  // 🟢 CORRECCIÓN 1: Nuevo estado para almacenar los productos ya enviados para el resumen
...
  
        if (isUserRegistered) {
          // 🟢 CORRECCIÓN 4: Usar la nueva función
          await fetchSubmittedProductData(); 
          setTimeout(() => {
            setIsLoading(false); 
          }, 300);
        } else {
...
  
    checkRegistration();
  }, [activeFair, sellerDtos, fetchSubmittedProductData, userId]);
  
...
        infoToPost.fairId
      );
      // 🟢 CORRECCIÓN 5: Usar la nueva función para refetch
      await fetchSubmittedProductData();
...
      // Remove all non-digit characters except for comma and period (and minus sign)
      let cleaned = priceString.replace(/[^\d.,-]/g, '');

      // Handle comma as decimal separator (replace with period)
      if (cleaned.includes(',')) {
        const parts = cleaned.split(',');
        // Keep the last comma as decimal separator, remove others
        cleaned = parts.slice(0, -1).join('') + '.' + parts.slice(-1);
      }

      // Ensure it's a number string
      const num = parseFloat(cleaned);
      if (isNaN(num)) return priceString; 

      // Format as currency string (e.g., adding $ or decimals if desired)
      // For simplicity in an input field, we just return the cleaned string representation
      return cleaned;
    };
...
    }
    
    // Apply capitalization only to non-price fields
    const newValue = field === 'price' ? value : capitalizedValue;
...
    // Use Intl.NumberFormat for cleaner formatting
    const numberValue = parseFloat(cleanedValue.replace(",", "."));
    
    if (isNaN(numberValue)) return value;

    // Format to Argentinian Peso style (dots for thousands, comma for decimal)
    return new Intl.NumberFormat('es-AR', {
      style: 'currency',
      currency: 'ARS',
      minimumFractionDigits: 0,
      maximumFractionDigits: 2,
    }).format(numberValue).replace('ARS', '').trim();
  };
  
...
          if (product.id === id) {
            const priceString = product.price.toString();
            // Reapply formatting after editing
            return {
              ...product,
              [field]: formatPrice(priceString),
            };
          }
          return product;
        })
...
  const liquidationSelected = "LIQUIDACION";
...
                            sellerId={userDtos?.seller?.id}
                            // 🟢 CORRECCIÓN 6: Pasar los productos enviados al componente de resumen
